...
class Client_GC():
    ...
    def compute_weight_update(self, local_epoch):
        """ For GCFL """
        copy(target=self.W_old, source=self.W, keys=self.gconvNames)#先将原来的卷积层（按gconvNames）权重拷贝下来

        train_stats = train_gc(self.model, self.dataLoader, self.optimizer, local_epoch, self.args.device)#和FedAvg一样训练

        subtract_(target=self.dW, minuend=self.W, subtrahend=self.W_old)#用更新的W-W_old得到差值dW(updates)

        self.train_stats = train_stats

        self.weightsNorm = torch.norm(flatten(self.W)).item()#权重的范数（W）

        weights_conv = {key: self.W[key] for key in self.gconvNames}
        self.convWeightsNorm = torch.norm(flatten(weights_conv)).item()#卷积层权重范数（conv W）

        dWs_conv = {key: self.dW[key] for key in self.gconvNames}#
        self.convDWsNorm = torch.norm(flatten(dWs_conv)).item()#权重更新dW（updates）的范数

        grads = {key: value.grad for key, value in self.W.items()}
        self.gradsNorm = torch.norm(flatten(grads)).item()#梯度范数（W的梯度）

        grads_conv = {key: self.W[key].grad for key in self.gconvNames}
        self.convGradsNorm = torch.norm(flatten(grads_conv)).item()#卷积层梯度范数（conv W的梯度）
...
def calc_gradsNorm(gconvNames, Ws):#计算卷积层的梯度norm
...
def train_gc_prox(model, dataloaders, optimizer, local_epoch, device, gconvNames, Ws, mu, Wt): #Ws: W ; Wt: W_old
    losses_train, accs_train, losses_val, accs_val, losses_test, accs_test = [], [], [], [], [], []
    convGradsNorm = []#
    train_loader, val_loader, test_loader = dataloaders['train'], dataloaders['val'], dataloaders['test']
    for epoch in range(local_epoch):
