...
def _randChunk(graphs, num_client, overlap, seed=None):
    ...
    totalNum = len(graphs)
    minSize = min(50, int(totalNum/num_client))#保证至少有50个数据
    graphs_chunks = []
    if not overlap:
        ...
        sizes = np.random.randint(low=50, high=150, size=num_client)#给每个client随机生成数据集大小，最小50
        for s in sizes:
            graphs_chunks.append(choices(graphs, k=s))#有放回随机抽样，一次取k个，因此有可能overlap
...
def prepareData_oneDS(datapath, data, num_client, batchSize, convert_x=False, seed=None, overlap=False):
    ...
    graphs_chunks = _randChunk(graphs, num_client, overlap, seed=seed)#给每个客户端分块
    splitedData = {}
    df = pd.DataFrame()
